...
def get_intersection(source_row, destination_df, column_name_node='poi_cbg',list_of_values_column_name='list_of_values'):
    intersection_counts = destination_df[list_of_values_column_name].apply(
        lambda list_destination: list_utils.get_intersection(source_row[list_of_values_column_name], list_destination, count=True)
    )

    intersection_df = pandas.DataFrame({
        f'{column_name_node}_source': source_row[column_name_node],
        f'{column_name_node}_destination': destination_df[column_name_node],
